...
def cmd_run(args, root: Path):
    plan_id = args.plan_id or time.strftime("plan-%Y%m%d-%H%M%S")
    cmd = ["python", "plan_and_run.py", "--root", str(root), "--task", args.task, "--plan-id", plan_id, "--mode", args.mode, "--policy", args.policy]
    if args.workspace:
        cmd.extend(["--workspace", args.workspace])
    subprocess.check_call(cmd, cwd=root)
    exec_dir = root / "artifacts" / "executions" / plan_id
...
def cmd_run_plan(args, root: Path):
    ...
    cmd = ["python", "-m", "services.controller_service", "--root", str(root), "--plan-id", args.plan_id, "--mode", args.mode, "--policy", args.policy]
    if workspace_arg:
...
def cmd_retry(args, root: Path):
    ...
    policy = meta.get("policy") or "guarded"

    exec_dir = root / "artifacts" / "executions" / plan_id
    before_run = find_latest_run(exec_dir)
    before_id = before_run.name if before_run else None
    cmd = ["python", "-m", "services.controller_service", "--root", str(root), "--plan-id", plan_id, "--mode", mode, "--policy", policy]
    if workspace_arg:
        cmd.extend(["--workspace", workspace_arg])
    subprocess.check_call(cmd, cwd=root)
...
def main():
    ...
    p_run.add_argument("--policy", default="guarded")
    ...
    p_run_plan.add_argument("--policy", default="guarded")
